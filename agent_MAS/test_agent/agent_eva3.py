...
class datasetc:
    ...
    def datagiv():
        ...
        datasetc.X_train, datasetc.X_test, datasetc.Y_train, datasetc.Y_test = train_test_split(datasetc.X, datasetc.Y,
                                                                                                test_size=test_size,
                                                                                                random_state=42)
...
class ClassifierML:
    ...
    def knnpima(self):
        ...
        # f1 score
        f1 = f1_score(datasetc.Y_test, y_pred)
        ...
        pack = [tavg[0], tavg[1], fpr, tpr, roc_auc,f1] #added f1 at the end
        return pack
    ...
    def nbayiris(self):
        ...
        # f1 score
        f1 = f1_score(datasetc.Y, y_pred)
        print(classification_report(y, y_pred))
        scores = cross_val_score(gnb, datasetc.X, datasetc.Y, cv=5)
        pack = [tavg[0], tavg[1], fpr, tpr, roc_auc,f1]
        return pack
    ...
    def ranfores(self):
        ...
        # f1 score
        f1 = f1_score(datasetc.Y_test, y_pred_en)
    ...
    def bagging(self):
        ...
        num_trees = 100
        model = BaggingClassifier(base_estimator=cart, n_estimators=num_trees, random_state=seed).fit(datasetc.X_train,
                                                                                                      datasetc.Y_train)
        thismopre = model.predict_proba(datasetc.X)
        tavg = np.average(thismopre, axis=0, weights=None)
        # accuracy
    ...
    def LDA(self):
        ...
        pack = [tavg[0], tavg[1], fpr, tpr, roc_auc, f1]  # F1-score'u paketinize ekleyin
        return pack
    ...
    def majorclassifire(self):
        ...
        eclf = VotingClassifier(
            estimators=[('knn', clf), ('gnb', clf2), ('dct', clf3), ('rf', clf4), ('abo', clf5), ('bcf', clf6)],
            voting='soft').fit(datasetc.X_train, datasetc.Y_train)
...
class CompContNet1(FipaContractNetProtocol):
    ...
    def handle_all_proposes(self, proposes):
        ...
        for message in proposes:
            ...
            print(content)
            power = np.array(content.get('model'))
            display_message(self.agent.aid.name,
                            'Analyzing Accuracy {i}'.format(i=content))
            i += 1
...
class CompContNet2(FipaContractNetProtocol):
    packageml=np.array([],dtype=float)
    '''CompContNet2
    ...
    def handle_cfp(self, message):
        ...
        All_data={'word':1,'model':self.modelmaker(self.mlid)}
